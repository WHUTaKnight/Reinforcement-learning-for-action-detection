    ...
    def _video_centric_sampling(self, video):
        ...
        def sample_video_proposals(proposal_type, video_id, video_pool, requested_num, dataset_pool):
            if len(video_pool) == 0:
                ...
                return [(dataset_pool[x], proposal_type) for x in
                        np.random.choice(len(dataset_pool), requested_num, replace=False)]
            else:
        ...
        out_props.extend(
            sample_video_proposals(1, video.id, incomp, self.incomplete_per_video, self.incomp_pool))  # sample incomp.
